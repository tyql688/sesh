...
- **恢复会话** — 在 Terminal、iTerm2、Ghostty、Kitty、Warp、WezTerm、Alacritty、Windows Terminal、PowerShell 中快速恢复（`⇧⌘R`）
- **实时监听** — 活跃会话更新时自动刷新（`⌘L`）
...
| Cursor CLI  | `~/.cursor/projects/*/agent-transcripts/**/*.jsonl` | JSONL  | 文件事件 |
| OpenCode    | `~/.local/share/opencode/opencode.db` | SQLite |   轮询   |
| Qwen Code   | `~/.qwen/projects/*/chats/*.jsonl`    | JSONL  | 文件事件 |
| CC-Mirror   | `~/.cc-mirror/{variant}/config/projects/**/*.jsonl` | JSONL  | 文件事件 |
